...
DEFCONFIG="${DEFCONFIG:-esp32s31_defconfig}"
...
DTB_FILE="${LINUX_DIR}/arch/riscv/boot/dts/espressif/esp32s31_generic.dtb"
...
cp -v "${DTB_FILE}"   "${TARGET_DIR}/esp32s31_generic.dtb"

echo ""
echo "=== Build complete ==="
echo "  Kernel : ${TARGET_DIR}/${TARGET}"
echo "  DTB    : ${TARGET_DIR}/esp32s31_generic.dtb"
echo "  Initrd : ${INITRAMFS_IMAGE} (${INITRAMFS_SIZE} bytes payload, ${INITRAMFS_PARTITION_SIZE} bytes flashed)"
echo "  Flash using: esptool -b 2000000 write-flash 0x2A0000 xipImage 0xA20000 initramfs.cpio"
ls -lh "${TARGET_DIR}/${TARGET}" "${TARGET_DIR}/esp32s31_generic.dtb" "${INITRAMFS_IMAGE}"
